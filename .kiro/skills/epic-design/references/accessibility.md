...
if (prefersReduced) {
  ...
  gsap.utils
    .toArray(".clip-reveal, .fade-reveal, .word-light")
    .forEach((el) => {
...
// Usage
splitTextAccessibly(document.querySelector(".hero-title"), {
  type: "chars,words",
});
```
...
let animationsEnabled = !window.matchMedia("(prefers-reduced-motion: reduce)")
  .matches;
