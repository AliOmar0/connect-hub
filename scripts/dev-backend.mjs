 ...
 *   wa    -> WhatsApp support backend    (whatsapp-support-backend, port 5000, heavy)
 *
...
requested.forEach((key, i) => {
  ...
  const pipe = (stream, isErr) => {
    ...
    stream.on('data', (chunk) => {
      buf += chunk.toString();
      if (svc.python && buf.includes('ModuleNotFoundError')) missingModule = true;
  ...
  child.on('exit', (code, signal) => {
    ...
    if (running <= 0 && !shuttingDown) {
      process.stdout.write(`${DIM}[launcher] All services have stopped.${RESET}\n`);
      process.exit(0);
    }
  });
  child.on('error', (err) => {
