...
describe("DashboardSidebar", () => {
  ...
  it("displays user email when profile is not loaded", async () => {
    ...
    await waitFor(() => {
      ...
      const userInfo = screen.queryByText(/test@example.com/i) ||
  ...
  it("toggles collapsed state", async () => {
    ...
    const buttons = screen.getAllByRole("button");
    const collapseButton = screen.queryByText("Collapse") ||
      buttons.find(btn => 
        btn.querySelector('svg')?.getAttribute('class')?.includes('chevron-left') ||
        btn.querySelector('svg')?.getAttribute('class')?.includes('chevron-right') ||
        btn.querySelector('svg')?.getAttribute('class')?.includes('ChevronLeft') ||
        btn.querySelector('svg')?.getAttribute('class')?.includes('ChevronRight')
      );
    ...
    if (collapseButton && collapseButton.isConnected) {
      ...
      const initialWidth = sidebar?.className.includes('w-64') ? 'w-64' : 'w-[72px]';
      ...
      await waitFor(() => {
        ...
        if (updatedSidebar) {
          const newWidth = updatedSidebar.className.includes('w-64') ? 'w-64' : 'w-[72px]';
