...
describe("SessionsTable", () => {
  const mockSessionsWithRelations: (Session & { customer?: Customer; employee?: Employee })[] = [
    {
      ...mockSessions[0],
  ...
  it("displays relative time for sessions", () => {
    ...
    const timeElements = Array.from(document.querySelectorAll("*")).filter((el) =>
      /ago|minute|hour|day/i.test(el.textContent || "")
    );
  ...
  it("calls onViewSession when row is clicked", () => {
    ...
    if (row) {
      fireEvent.click(row);
      expect(mockOnViewSession).toHaveBeenCalledWith(mockSessionsWithRelations[0]);
    }
  });
  ...
  it("shows dropdown menu with actions", async () => {
    ...
    const assignAgent = await screen.findByText("Assign Agent", { timeout: 3000 });
  ...
  it("calls onViewSession when 'View Details' is clicked", async () => {
    ...
    const viewDetailsButton = await screen.findByText("View Details", { timeout: 3000 });
  ...
  it("calls onAssignAgent when 'Assign Agent' is clicked", async () => {
    ...
    const assignAgentButton = await screen.findByText("Assign Agent", { timeout: 3000 });
    await user.click(assignAgentButton);
    
    expect(mockOnAssignAgent).toHaveBeenCalledWith(mockSessionsWithRelations[0]);
  });
  ...
  it("displays different status styles for different statuses", () => {
    const statuses = ["active", "waiting", "completed", "escalated", "missed"] as const;
  ...
  it("displays different channel icons for different channels", () => {
    const channels = ["whatsapp", "messenger", "sms", "voice", "email"] as const;
